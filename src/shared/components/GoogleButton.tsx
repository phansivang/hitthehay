 ...
 * Provides a button for Google Sign-In
 */
...
export const GoogleButton: React.FC<GoogleButtonProps> = ({
  ...
  const { isLoaded, isLoading, error, signIn } = useGoogleAuth({
    onSuccess,
    onError,
    autoLoad: true,
  });
  ...
  const handleClick = () => {
    if (!isLoaded) {
      onError?.(new Error('Google authentication is not ready yet. Please try again in a moment.'));
      return;
    }
    signIn();
  };
