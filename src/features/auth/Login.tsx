...
const Login = () => {
  ...
  const handleOAuthSuccess = async (credentials: OAuthCredentials) => {
    try {
      const response = await authService.loginWithOAuth(credentials);
      signIn(response.user);
      navigate('/editor');
    } catch (error) {
      console.error(`${credentials.provider} login error:`, error);
      // Error is handled by OAuthButton component
    }
  };
