...
import { ChevronLeft, Settings, User, LogOut, Zap } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ allowedCategories }) => {
  ...
  const categorizedNodes = Object.values(NodeCategory).map(category => ({
    category,
    nodes: NODE_LIBRARY.filter(node => node.category === category)
  }));
