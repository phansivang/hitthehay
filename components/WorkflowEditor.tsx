...
// This inner component is wrapped by ReactFlowProvider, so it can safely use React Flow hooks.
const InnerEditor = () => {
  ...
  const allowedCategories = useMemo(() => {
    ...
    if (!hasNodeFromCategory(NodeCategory.Triggers)) return []; // Should not happen in normal flow
    ...
    (event: React.DragEvent, reactFlowWrapper: React.RefObject<HTMLDivElement>) => {
      ...
      if (nodes.length > 0) {
        if (newNode.data.category === NodeCategory.Platforms) {
          // If the new node is a platform, connect it to the last non-platform node (fan-out)
          sourceNode = [...nodes].reverse().find(n => n.data.category !== NodeCategory.Platforms) || null;
        } else {
          // Otherwise, connect sequentially to the last node
